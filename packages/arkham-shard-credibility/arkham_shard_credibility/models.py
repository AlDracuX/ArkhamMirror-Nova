...
class DeceptionChecklist:
    ...
    def calculate_score(self) -> int:
        ...
        assessed_indicators = [ind for ind in self.indicators if ind.strength != IndicatorStrength.NONE]

        if not assessed_indicators:
            return 0

        # Calculate score based only on assessed indicators
        total = sum(strength_scores.get(ind.strength, 0) * ind.confidence for ind in assessed_indicators)
