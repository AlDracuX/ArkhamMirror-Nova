...
class MetadataForensicAnalyzer:
            ...
            with Image.open(file_path) as img:
                ...
                for exif_field, attr in datetime_fields:
                    if exif_field in raw_data:
                        try:
                            dt = datetime.strptime(raw_data[exif_field], "%Y:%m:%d %H:%M:%S")
    ...
    def _convert_gps_coords(self, coords: Optional[Tuple], ref: Optional[str]) -> Optional[float]:
        """
                ...
                if "docProps/core.xml" in zf.namelist():
                    with zf.open("docProps/core.xml") as f:
                        ...
                        if created is not None and created.text:
                            try:
                                office.created = datetime.fromisoformat(created.text.replace("Z", "+00:00"))
                            except ValueError:
                                pass
                        ...
                        if modified is not None and modified.text:
                            try:
                                office.modified = datetime.fromisoformat(modified.text.replace("Z", "+00:00"))
                            except ValueError:
                                pass
    ...
    def analyze_integrity(
        ...
        if exif:
            ...
            if exif.software:
                editing_tools = ["photoshop", "gimp", "lightroom", "snapseed", "picasa", "afterlight", "vsco"]
        ...
        if pdf:
            ...
            if pdf.producer:
                manipulation_tools = ["ghostscript", "pdftk", "pdf-tools", "itext", "fpdf", "reportlab"]
    ...
    def compare_documents(
        ...
        if scan1.exif_data and scan2.exif_data:
            ...
            if (
                exif1.make
                and exif2.make
                and exif1.make == exif2.make
                and exif1.model
                and exif2.model
                and exif1.model == exif2.model
            ):
                similarities.append(
                    {
                        "type": "same_camera",
        ...
        if similarities and not comparison.match_score:
            comparison.match_score = sum(s.get("confidence", 0.5) for s in similarities) / len(similarities)
    ...
    def full_scan(
        ...
        try:
            ...
            if any(x in mime_lower for x in ["word", "document", "spreadsheet", "presentation", "officedocument"]):
