...
function TreeListView() {
  ...
  return (
    <div>
      <div
        style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px', marginBottom: '20px' }}
      >
        <button
      ...
      {trees.length === 0 ? (
        <div
          style={{
            ...
            borderRadius: '12px',
            border: '1px dashed var(--arkham-border, #e5e7eb)',
          }}
        >
          <Icon name="GitBranch" size={48} />
          <p style={{ marginTop: '16px', fontSize: '16px' }}>No question trees found.</p>
          <p style={{ fontSize: '14px' }}>
            Create or generate a tree to start witness preparation.
          </p>
        </div>
      ) : (
        <div
      ...
      >
        <div style={{ flex: 1 }}>
          ...
          >
            ...
            {String(tree.witness_name || tree.witness_id || 'Unknown Witness')}
          </div>
        </div>
        <div
          style={{
            padding: '4px 8px',
            borderRadius: '6px',
            background: `${getDamageColor(damageScore)}15`,
            color: getDamageColor(damageScore),
            fontSize: '12px',
            fontWeight: 700,
            border: `1px solid ${getDamageColor(damageScore)}30`,
            whiteSpace: 'nowrap',
          }}
        >
      ...
      >
        <div>
          <h1 style={{ fontSize: '24px', fontWeight: 600, margin: '0 0 8px 0' }}>
            Question Tree Detail
          </h1>
          <p style={{ color: 'var(--arkham-text-muted, #6b7280)', margin: 0, fontSize: '15px' }}>
            Witness:{' '}
            <strong style={{ color: 'var(--arkham-text-primary)' }}>
              {String(nodes[0]?.witness_name || 'Witness')}
            </strong>
          </p>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
          ...
          >
            ...
            <div style={{ fontSize: '13px', color: '#166534' }}>
              {String(node.expected_answer || 'None specified')}
            </div>
          </div>
          <div
            style={{
          ...
          >
            ...
            <div style={{ fontSize: '13px', color: '#991b1b' }}>
              {String(node.alternative_answer || 'None specified')}
            </div>
          </div>
        </div>
      </div>
...
function ImpeachmentCard({ impeachment }: { impeachment: Record<string, unknown> }) {
  ...
  return (
    <div
      ...
      onMouseEnter={(e) => {
        e.currentTarget.style.background = 'var(--arkham-bg-tertiary, #f9fafb)';
      }}
      onMouseLeave={(e) => {
        e.currentTarget.style.background = 'var(--arkham-bg-secondary, white)';
      }}
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
        ...
        >
          ...
          {String((impeachment.steps as Array<unknown>)?.length || 0)} steps
        </div>
    ...
    <div style={{ padding: '24px', maxWidth: '900px', margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: '32px' }}>
        ...
        <p style={{ color: 'var(--arkham-text-muted, #6b7280)', margin: 0, fontSize: '15px' }}>
          Witness:{' '}
          <strong style={{ color: 'var(--arkham-text-primary)' }}>
            {String(impeachment.witness_name || 'Unknown')}
          </strong>{' '}
          | Conflict detected via Statement vs Document
        </p>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
        ...
        {steps.length === 0 ? (
          <p style={{ color: 'var(--arkham-text-muted)', textAlign: 'center', padding: '32px' }}>
            No sequence steps defined for this impeachment.
          </p>
        ) : (
                    ...
                    >
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                        ...
                        <div style={{ borderLeft: '1px solid #ffedd5', paddingLeft: '16px' }}>
                          ...
                          >
                            Reference:{' '}
                            {String((step.conflict as Record<string, unknown>).reference)}
                          </div>
                        </div>
                      </div>
                    </div>
      ...
      >
        ...
        <label style={{ display: 'block', marginBottom: '24px' }}>
          ...
          <textarea
            ...
            rows={3}
            placeholder="Goal of this line of questioning..."
            style={{
              width: '100%',
              padding: '10px',
              borderRadius: '6px',
...
function GenerateTreeDialog({
  ...
  const handleGenerate = async () => {
    if (!witnessId.trim()) {
      toast.error('Witness selection is required');
      return;
    }
    try {
      setGenerating(true);
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Our AI analyst will examine all case documents related to the selected witness to identify
          inconsistencies and build a damaging line of questioning.
        </p>
