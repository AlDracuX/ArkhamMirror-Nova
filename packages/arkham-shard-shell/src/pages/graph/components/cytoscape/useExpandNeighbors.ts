...
export function useExpandNeighbors(
   ...
   */
  const setSeedNodes = useCallback(
    (nodeIds: string[]) => {
  ...
  const expandNode = useCallback(
    async (nodeId: string) => {
      ...
      try {
        ...
        const sourcePos: Position = sourceNode.length > 0 ? sourceNode.position() : { x: 0, y: 0 };
   ...
   */
  const getExpansionIndicator = useCallback(
    (nodeId: string): 'expandable' | 'expanded' | 'max-depth' | 'seed' => {
    ...
    if (cy) {
      cy.batch(() => {
        cy.nodes().removeClass(
          'seed-node expanded expandable fully-expanded collapsed-neighbor newly-expanded'
        );
   ...
   */
  const getNodesAtDepth = useCallback(
    (depth: number): string[] => {
      const result: string[] = [];
