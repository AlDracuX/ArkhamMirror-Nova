...
export function AuditTrailPage() {
  ...
  const [activeTab, setActiveTab] = useState<TabKey>(
    (searchParams.get('tab') as TabKey) || 'actions'
  );
      ...
      >
        ...
        <button
          ...
          style={{
            ...
            gap: '8px',
            padding: '10px 20px',
            background: 'var(--arkham-bg-accent, #059669)',
            color: 'white',
            border: 'none',
            borderRadius: '8px',
            cursor: 'pointer',
      ...
      >
        ...
        {activeTab === 'actions' && (
          ...
          >
            ...
            <button
              onClick={() =>
                setFilters({ user_id: '', action_type: '', date_from: '', date_to: '' })
              }
              style={{
                marginTop: '18px',
                padding: '8px 12px',
...
function StatCard({
  ...
  return (
    ...
    >
      ...
      <div>
        <div
          style={{ fontSize: '13px', color: 'var(--arkham-text-muted, #6b7280)', fontWeight: 500 }}
        >
          {label}
        </div>
  ...
  return (
    <button
      ...
      style={{
        ...
        borderBottom: active
          ? '2px solid var(--arkham-bg-accent, #059669)'
      ...
      {!!count && (
        <span
          style={{
            padding: '2px 8px',
            borderRadius: '12px',
            background: active ? '#05966915' : '#f3f4f6',
            fontSize: '12px',
            fontWeight: 600,
          }}
        >
...
function FilterField({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
      <label
        style={{ fontSize: '12px', fontWeight: 600, color: 'var(--arkham-text-muted, #6b7280)' }}
      >
        {label}
      </label>
      {children}
  ...
  if (actions.length === 0) {
    return (
      <EmptyState
        ...
        desc="Try adjusting your filters or checking a different date range."
      />
    );
  }
        ...
        <tbody>
          {actions.map((action, idx) => (
            ...
            >
              ...
              >
                {
                  String(action.created_at || action.timestamp || '')
                    .replace('T', ' ')
                    .split('.')[0]
                }
              </td>
              ...
              <td style={{ padding: '14px 24px' }}>
                <span
                  style={{
                    ...
                    background: getActionColor(String(action.action_type)) + '15',
                    color: getActionColor(String(action.action_type)),
                  }}
                >
              ...
              <td style={{ padding: '14px 24px', maxWidth: '400px' }}>
                ...
                >
                  {String(
                    action.description || action.details || JSON.stringify(action.metadata || {})
                  )}
                </div>
              </td>
            </tr>
  ...
  if (sessions.length === 0)
    return (
      <EmptyState
        ...
        desc="User sessions will appear here as they interact with the system."
      />
    );
  return (
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            ...
            <div>
              ...
              <div style={{ fontSize: '13px', color: 'var(--arkham-text-muted, #6b7280)' }}>
                User: {String(session.user_id || 'unknown')}
              </div>
            </div>
          </div>
          <div style={{ display: 'flex', gap: '48px', alignItems: 'center' }}>
            <div style={{ textAlign: 'right' }}>
              ...
              <div style={{ fontSize: '13px', fontWeight: 500 }}>
                {String(session.start_time || '').split('T')[0]}{' '}
                <span style={{ color: '#9ca3af', margin: '0 4px' }}>→</span>{' '}
                {String(session.end_time || 'Active').split('T')[0]}
              </div>
            </div>
  ...
  if (exports.length === 0)
    return (
      <EmptyState
        ...
        desc="History of generated reports and data extracts will appear here."
      />
    );
  return (
    <div
        ...
        >
          <div
            style={{
              ...
              right: 0,
              height: '4px',
              background: getFormatColor(String(exp.export_format)),
            }}
          />
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '16px' }}>
            ...
            >
              {String(exp.export_format)}
            </span>
          </div>
          <div style={{ fontWeight: 600, fontSize: '15px', marginBottom: '4px' }}>
            Export #{String(exp.export_id || exp.id).substring(0, 8)}
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            ...
            >
              <Icon name="Clock" size={14} /> {String(exp.created_at || '').split('T')[0]}
            </div>
            <div
              style={{
                display: 'flex',
            ...
            >
              <Icon name="User" size={14} /> {String(exp.user_id || 'system')}
            </div>
            <div
              style={{
                display: 'flex',
            ...
            >
              <Icon name="Layers" size={14} /> {String(exp.row_count || '0')} records
            </div>
          </div>
          <div
            style={{
          ...
          >
            ...
            >
              <Icon name="Eye" size={14} /> View Filters
            </button>
          </div>
        </div>
      ))}
