...
export async function listItems(
  projectId?: string,
  status?: string
): Promise<SkeletonListResponse> {
  const params = new URLSearchParams();
  if (projectId) params.set('project_id', projectId);
  if (status) params.set('status', status);

  const query = params.toString();
  return fetchAPI<SkeletonListResponse>(`/items${query ? `?${query}` : ''}`);
}

export async function getItem(itemId: string): Promise<SkeletonItem> {
  return fetchAPI<SkeletonItem>(`/items/${itemId}`);
}
