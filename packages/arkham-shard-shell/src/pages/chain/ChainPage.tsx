    ...
    <div style={{ padding: '24px', maxWidth: '1200px', margin: '0 auto' }}>
      ...
      >
        ...
        <StatCard
          ...
          value={String(
            items.filter((it) => !it.metadata?.integrity || it.metadata?.integrity === 'unverified')
              .length
          )}
          icon="Clock"
          color="#6b7280"
        ...
        >
          ...
          <p style={{ color: '#6b7280', fontSize: '14px' }}>
            {search
              ...
              : 'Get started by creating your first evidence item'}
          </p>
        </div>
      ) : (
...
function ItemDetailView({ itemId, onBack }: { itemId: string; onBack: () => void }) {
  ...
  const loadDetail = useCallback(async () => {
    try {
      ...
      setHistory(
        Array.isArray(historyData.history) ? (historyData.history as Record<string, unknown>[]) : []
      );
      setReports(
        Array.isArray(reportsData.reports) ? (reportsData.reports as Record<string, unknown>[]) : []
      );
    } catch (err) {
  ...
  if (!item)
    return (
      <div style={{ padding: '40px', textAlign: 'center' }}>
        Item not found. <button onClick={onBack}>Go back</button>
      </div>
    );

  const currentHash = String(
    integrityResult?.current_hash ||
      (item.metadata as Record<string, unknown>)?.hash ||
      'Not hashed'
  );
    ...
    <div style={{ padding: '24px', maxWidth: '1200px', margin: '0 auto' }}>
      ...
      >
        ...
        <div style={{ display: 'flex', gap: '10px' }}>
          ...
          >
            <Icon name="Shield" size={16} color={verifying ? '#9ca3af' : '#3b82f6'} />{' '}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '24px' }}>
        <div>
                ...
                status={
                  integrityResult
                    ? integrityResult.valid
                      ? 'verified'
                      : 'tampered'
                    : ((item.metadata as Record<string, unknown>)?.integrity as any) || 'unverified'
                }
                large
              />
            </div>
              ...
              {!!integrityResult && (
                <div
                  style={{
                    ...
                    alignItems: 'center',
                    gap: '6px',
                    color: integrityResult.valid ? '#10b981' : '#ef4444',
                  }}
                >
          ...
          <div style={{ paddingLeft: '20px', borderLeft: '2px solid #e5e7eb', marginLeft: '10px' }}>
            {history.length === 0 ? (
              <p style={{ color: '#9ca3af', fontSize: '14px', padding: '10px 0' }}>
        ...
        <div>
          ...
          >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              <StatusItem
                label="Created By"
                value={String(item.created_by || 'Unknown')}
                icon="User"
              />
            </div>
          </div>
                  ...
                  >
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: '13px', fontWeight: 600 }}>
                        Report — {String(report.report_id || 'ID Unknown').substring(0, 8)}
                      </div>
                      <div style={{ fontSize: '11px', color: '#9ca3af' }}>
                        Generated {formatDate(String(report.created_at || ''))}
                      </div>
                    </div>
...
function EvidenceRow({ item, onClick }: { item: ExtendedChainListItem; onClick: () => void }) {
  const integrity = String(item.metadata?.integrity || 'unverified') as
    | 'verified'
    | 'unverified'
    | 'tampered';
    ...
    <div style={{ position: 'relative', paddingBottom: isLast ? 0 : '24px' }}>
      ...
      >
        ...
        <div style={{ fontSize: '12px', color: '#9ca3af' }}>
          {formatDate(String(event.timestamp || event.created_at || ''))}
        </div>
      </div>
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          background: 'var(--arkham-bg-primary, #ffffff)',
          padding: '24px',
          borderRadius: '12px',
          width: '450px',
          maxWidth: '95vw',
